...
Ec2b::Ec2b(int method) {
	...
	char _data[2048];
	switch(method) {
	case Ec2b::SEED_FROM_ZERO:
		memset(_key, 0, 16);
		memset(_data, 0, 2048);
		break;
	case Ec2b::SEED_FROM_RAND:
		getrandom(_key, 16, 0);
		getrandom(_data, 2048, 0);
		break;
	default:
		if ((method & 0xff) == Ec2b::SEED_FROM_CONST_HIGH_BYTE) {
			unsigned int v = (method >> 8) & 0xff;
			memset(_key, v, 16);
			memset(_data, v, 2048);
			break;
		}
...
void Ec2b::getFromSeed(int method) {
	unsigned char _d[2048];
	...
	unsigned int i;
	switch(method) {
	case Ec2b::SEED_FROM_OBJECT:
		assert(key.size() >= 16);
		memcpy(_k, key.c_str(), 16);
		assert(data.size() >= 2048);
		memcpy(_d, data.c_str(), 2048);
		break;
	case Ec2b::SEED_FROM_RAND:
		getrandom(_k, 16, 0);
		getrandom(_d, 2048, 0);
		data.assign((const char*) _d, 2048);
		break;
	case Ec2b::SEED_FROM_SEED:
	case Ec2b::SEED_FROM_SEED_MT:
		init_genrand64(seed);
		k[0] = genrand64_int64();
		k[1] = genrand64_int64();
		for (i = 0; i < 256; i++) {
		...
		data.assign((const char*) _d, 2048);
		break;
	case Ec2b::SEED_FROM_SEED_XOR:
		xorseed[0] = seed;
		k[0] = rand_xoshiro256(xorseed);
		k[1] = rand_xoshiro256(xorseed);
		for (i = 0; i < 256; i++) {
			d[i] = rand_xoshiro256(xorseed);
		}
		data.assign((const char*) _d, 2048);
		break;
	case Ec2b::SEED_FROM_ZERO:
...
extern "C" {
	...
	static const unsigned char mul2Tbl[256] = {
	...
	static const unsigned char mul3Tbl[256] = {
	...
	static const unsigned char shiftTblInv[16] = {
	...
	static void subBytesInv(unsigned char key[16]) {
		unsigned int i;
		for (i = 0; i < 16; i++) {
			key[i] = sboxInv[key[i]];
		}
	...
	static void shiftRowsInv(unsigned char key[16]) {
		unsigned int i;
		char key2[16];
		memcpy(key2, key, 16);
		for (i = 0; i < 16; i++) {
	...
	static void mixColInv(unsigned char key[4]) {
		unsigned char a0 = key[0];
		unsigned char a1 = key[1];
		unsigned char a2 = key[2];
		unsigned char a3 = key[3];
	...
	static void mixColsInv(unsigned char _key[16]) {
		unsigned char key[4][4];
		memcpy(key, _key, 16);
		unsigned int i;
		for (i = 0; i < 4; i++) {
	...
	static void unscrambleKey(unsigned char key[16]) {
		unsigned int i;
		for (i = 0; i < 16; i++) {
			key[i] ^= keyXorpad[i];
		}
		...
		subBytesInv(key);
		for (i = 1; i < 10; i++) {
			xorRoundKey(key, aesRoundKeys[10 - i]);
			mixColsInv(key);
			shiftRowsInv(key);
			subBytesInv(key);
		}
